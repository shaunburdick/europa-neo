...
### v1.4 (2026-09-06) — Flow-viable city connectivity invariant (INV-16)

- **New invariant INV-16**: the generator now validates that every city can reach every other city via **flow-viable** land edges (unidirectional pipe viability: `flowRateForDelta(delta, ENGINE_CONSTANTS) > 0` OR `flowRateForDelta(-delta, ENGINE_CONSTANTS) > 0` for each traversed edge — at least one direction must move troops). This catches maps where land connectivity (INV-12) passes but extreme elevation gradients block pipe flow, rendering the map unplayable.
- **Trigger**: user report of seed 586441717 — a structurally valid map (passes INV-1..INV-15) with cities at opposite corners (0,0) and (31,31), max adjacent elevation delta 16, 30.79% stalled edges, and only 0.22% flow-viable reachable land. The map is functionally unplayable because troops cannot cross the board.
