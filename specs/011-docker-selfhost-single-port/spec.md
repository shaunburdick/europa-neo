...
- **NFR-004 (Security)**: The single `http.Server` MUST retain existing static-UI hardening: path-traversal guard (`isPathInside` + `realpath` containment), `STATIC_SECURITY_HEADERS` baseline, `/version` security headers, no credential-bearing URL acceptance, and no opaque-id/session-token leakage in logs (spec 010 NFR-003/FR-024 boundary preserved).
