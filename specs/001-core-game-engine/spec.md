...
2. **Given** a saturated city with an eastward pipe into an empty cell, **When** ticks elapse, **Then** troops accumulate in the destination cell while the source is decremented by the amount transferred (Clarifications v1.6 — flow is a transfer, not a copy; the source does NOT stay at capacity).
