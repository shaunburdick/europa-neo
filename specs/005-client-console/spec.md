...
2. **Given** the sidebar, **When** it renders, **Then** it contains Status (tick counter, player turn, connection), Players (list with color indicators, names, roles), Orders (exclusive/clear toggle with mode label), Reserve (percentage slider + quick-select buttons 0/25/50/75/100), Overview (minimap with viewport rectangle), Zoom (level indicator + controls), Surrender button, and Help button — in that vertical order.
...
- **FR-015 (issue #76)**: The sidebar MUST contain, in vertical order: Status (tick counter, player turn indicator, connection status), Players (list with color indicators, names, and roles YOU/P2/...), Orders (exclusive/clear mode toggle with mode label), Reserve (percentage slider with quick-select buttons at 0, 25, 50, 75, 100), Overview (minimap with viewport rectangle), Zoom (level indicator and controls), Surrender button, and Help button.
...
- **FR-018 (issue #76)**: The console MUST provide keyboard zoom shortcuts: `+` or `=` zooms in one step, `-` or `_` zooms out one step, `0` resets to 100%. Shortcuts MUST be suppressed when focus is inside interactive chrome (buttons, inputs, toolbars, contenteditable) — reusing the existing `HotkeyController` focus guard.
...
- **Keyboard shortcuts**: `HotkeyController` extended with a UI-zoom layer; `+`/`=` zooms in, `-`/`_` zooms out, `0` resets to 100%. Suppressed when focus is inside interactive chrome (reuses existing focus guard).
...
- **Data model addition**: `CellRenderInfo` gains an additive `pipeIntensities: ReadonlyMap<Direction, number>` field (0–1 normalized). Both contract mirrors updated. Consumers that don't render intensity may ignore it.
- **CSS rendering**: pipe spans carry `data-slope` attribute selectors mapping to `--europa-pipe-color` design tokens. Intensity is computed in JavaScript (cell-view.tsx) and applied as `--europa-pipe-tri` CSS custom property via inline style.
- **Canvas rendering**: triangle size scales by `(0.4 + intensity * 0.6)`. Stalled pipes remain full size with hollow stroke.
- **Web component**: `EuropaPipeSlope` gains an optional `intensity` attribute (0–1, default 1). Triangle border-widths scale proportionally. Stalled pipes ignore intensity. `aria-label` includes a qualitative qualifier ("light"/"moderate"/"strong") when intensity < 1.
...
### Layout acceptance mapping (issue #76 sidebar + zoom)

| Acceptance item | Proving suite | Result |
| --- | --- | --- |
