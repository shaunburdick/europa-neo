...
- **FR-009**: A drift check (CI + local script) MUST assert `APP_VERSION` === root `package.json` version === every workspace `package.json` version === the version strings shown in the README header, manual index footer, manual layout footer, and `DESIGN.md` version header. On failure it MUST exit non-zero and name every disagreeing file. The fixed root/document surfaces must each be observed exactly once; a partial observation set is not a successful check.
