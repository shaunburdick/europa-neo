...
- 2026-08-26 (feature 010 Clarifications v1.6): the wire-mirrored `IdentityState` gains an additive OPTIONAL `guestPlayerId?: GuestPlayerId` — carried ONLY on the directed `identity` lobby event delivered to its OWNING connection, as the FR-003 resume-claim delivery channel (the browser persists it so a reload restores the active identity within grace). Listings, snapshots, join/spectate targets, UI, URLs, and logs remain strictly id-free; no version bump (same additive ruling pattern as this section's `detail` record). Recorded here because this ruling's "claims advisory" summary is where a reader would otherwise conclude the wire never returns the opaque id.
