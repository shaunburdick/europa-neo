...
- **Shadow DOM vs Light DOM (two-tier DOM model, per Clarifications v1.1)**: generic child-projecting components use **Shadow DOM (open) + `<slot>` projection** — children remain light-DOM children of the host and are projected by the browser, never reparented by script — with one shared constructed `CSSStyleSheet` adopted via `adoptedStyleSheets`. Game-specific primitives remain **Light DOM**. No `::part()` theming API. This preserves the single-stylesheet rule (FR-011) and all existing styling.
- **Focus trap across the shadow boundary**: the modal's focus trap runs via event listeners and MUST collect focusables from the shadow tree AND `slot.assignedElements()` subtrees (including nested open-shadow components), resolving the deep active element for boundary comparisons and focus restore. The trap MUST cycle Tab/Shift+Tab within the flattened dialog tree and restore focus to the trigger element on close.
...
- **FR-007**: The stylesheet (`dist/design.css`) MUST continue to be imported separately by the consumer (existing single-stylesheet contract, FR-011 of spec 012). The web components do NOT auto-import the stylesheet. The `@europa/design/components` export contains JavaScript only — no CSS file in the export map. Shadow-DOM components (FR-009) additionally embed the catalog class rules as a generated TypeScript string module (`src/styles/catalog-styles.ts`, emitted by `build-css.ts` from the same authored source) and adopt them as a constructed stylesheet — that is JavaScript, not a stylesheet artifact; consumers MUST still load `design.css` because the `:root` token definitions it carries are what feed CSS custom properties through the shadow boundaries.
...
#### DOM model and class catalog composition

- **FR-009**: Web components use a **two-tier DOM model** (per Clarifications v1.1). Generic child-projecting components (FR-001) MUST use **Shadow DOM (open) + `<slot>` projection**: children remain light-DOM children of the host and are projected by the browser, and the component MUST adopt one shared constructed `CSSStyleSheet` via `adoptedStyleSheets` (catalog class rules only — the `:root` token block is excluded because CSS custom properties inherit through shadow boundaries). Game-specific primitives (FR-002) MUST remain **Light DOM**, applying catalog classes and inline token styles directly. All components still compose the exact `europa-*` catalog classes, preserving the single-stylesheet rule (FR-011 of spec 012): `design.css` remains the single stylesheet file, loaded by the consumer.
- **FR-009a**: Manually reparenting host children (e.g. `appendChild` loops moving light-DOM children into internal wrappers) is FORBIDDEN in any component. It breaks React 19 unmounting (`removeChild` NotFoundError) — React does not support web components mutating React-managed child nodes — and is the crash the Shadow DOM decision exists to fix.
...
- **FR-015**: All interactive web components (button, modal controls) MUST participate in the shared focus-visible treatment via the `*:focus-visible` rule in `design.css` (delivered into shadow roots by the adopted catalog stylesheet for Shadow-DOM components — same source rule). No component may suppress or override the focus ring.
...
- The DOM model is the two-tier decision of Clarifications v1.1 (superseding the original all-light-DOM product-owner decision 1): Shadow DOM (open) + slots + one shared adopted stylesheet for generic child-projecting components; Light DOM for game primitives. `design.css` remains the consumer-loaded stylesheet of record; the adopted sheet carries only the catalog class rules.
...
- **`::part()` theming API or per-component style isolation**: Shadow DOM (open) is used for child projection and shared-rule adoption (Clarifications v1.1), but no `::part()`/`::theme` API and no per-component style forks ship.
...
- **Q1 — Why change the DOM model?** React 19 crashed on unmount (`removeChild` NotFoundError) because the light-DOM components reparented React-managed children into internal wrappers via `appendChild` loops. React does not support web components mutating React-managed child nodes; its documented recommendation is Shadow DOM + `<slot>` projection. Decision change approved by the product owner.
- **Q2 — What is the new DOM model?** Two tiers (FR-009/FR-009a). The 13 generic components (page, card, plate, stack, container, badge, grid, banner, chip, typography, waiting, button, modal) use **Shadow DOM (open) + `<slot>`**: children stay light-DOM children of the host and are projected by the browser. The 7 game primitives (troop-chip, city-marker, pipe-slope, elevation-swatch, player-badge, fog-overlay, reserve-indicator) remain **Light DOM** — leaf elements with no child projection. Manual reparenting of host children is forbidden (FR-009a).
- **Q3 — How is styling delivered?** One authored source (`src/styles/catalog.css`) is compiled by `scripts/build-css.ts` into `dist/design.css` (global stylesheet — contract unchanged, still loaded by the consumer for the `:root` token definitions, light-DOM primitives, and plain HTML) and the gitignored generated module `src/styles/catalog-styles.ts` (catalog class rules only; `:root` excluded — custom properties inherit through shadow boundaries). `EuropaElement.ensureShadowRoot()` lazily attaches an open shadow root and adopts ONE shared constructed `CSSStyleSheet` via `adoptedStyleSheets`.
- **Q4 — Does FR-011's modal contract change?** No. The focus trap was rewritten for the flattened tree: it collects shadow-tree focusables plus `slot.assignedElements()` subtrees (including nested open-shadow components) and resolves the deep active element for boundary comparisons and focus restore. All FR-011 obligations (a)–(g) are unchanged.
- **Q5 — What happened to the button's MutationObserver?** Removed, along with the `_updating` guard and `_renderQueued` microtask queue — slot projection makes reparenting-timing handling unnecessary. `formAssociated` + `attachInternals()` + `requestSubmit()` remain, with the click handler on the host element.
...
- **Principle V (Simplicity)**: Thin wrappers (~50–100 LOC each). No framework, no state management; the Shadow DOM pattern is a fixed ~10-line base-class helper (`ensureShadowRoot()`), and the most complex component (modal) has ~80 LOC for the flattened-tree focus trap + Escape + focus-restore.
