...
#### Documentation adoption — Astro via vendored/shared stylesheet, scope preserved
...
- **FR-013**: The manual's Pages integration MUST preserve the artifact scope guarantees from `.github/workflows/pages-deploy.yml`: the Astro build outputs only rendered manual HTML under `docs/manual/dist`, and the uploaded artifact remains exactly that tree. The shared stylesheet MUST reach the Pages deployment by being **vendored/copied into `docs/manual`** (e.g., `docs/manual/public/design.css` for the Astro-served copy and `docs/manual/assets/design.css` as the G-05 target) that is checked into the source tree from the design build, not by widening the workflow's `source` or `path` to include `packages/**` or repo-root. FR-013 is a correctness guard: artifact contents MUST be auditable from the `docs/manual` tree alone.
...
- **FR-015**: The manual's MDX-to-HTML path MUST NOT introduce an external theme or CDN dependency. The Astro layout pulling the vendored `public/design.css` from the manual's own tree is allowed; adding an Astro theme, a theme gem, or a `<link>` to a CDN is not.
...
### v1.5 (2026-09-12) — Identity-decoupled color props (issue #74 companion)

- **Supersedes v1.4's derivation clause**: v1.4 stated that design components "MUST derive player colors from these tokens rather than duplicating the hex values." Under issue #74 a player's identity is an opaque canonical `PlayerId` with no numeric `1 | 2 | 3 | 4` ordering, so the presentational primitives no longer own an identity → color map. `EuropaTroopChip`, `EuropaCityMarker`, and `EuropaPlayerBadge` now accept an explicit `color` (CSS color string) prop, and the caller passes a `playerColor1`–`playerColor4` token value. The **single-source invariant is unchanged and strengthened**: no package duplicates the player-color hex values; the caller reads them from `TOKENS`. `playerColor1`–`playerColor4` remain canonical design tokens in the `DESIGN.md` token table. The design package gains no `@europa/core` dependency.
- **Breaking prop change (pre-1.0 minor)**: the numeric `owner`/`player` props are deleted (spec 014 v1.4). This is additive to the design system's token contract (no token/class changes) but breaking to the three components' props; the affected consumers (Astro manual MDX, design dev page) are updated in the same change set.
