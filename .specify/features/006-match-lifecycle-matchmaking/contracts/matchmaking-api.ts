...
// ----------------------------------------------------------------------------
// Server dependencies (re-declared from networking)
// ----------------------------------------------------------------------------
//
// We re-declare the `Server` and `Logger` types here (rather than
// importing from `@europa/networking`) so the matchmaker's contract file
// is self-contained. The conformance test (`tests/conformance.test.ts`)
// imports the actual `Server` from `@europa/networking` and asserts the
// shapes match — any drift between this re-declaration and the canonical
// networking declaration fails CI.
// ----------------------------------------------------------------------------
