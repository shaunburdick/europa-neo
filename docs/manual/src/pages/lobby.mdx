...
The lobby is Europa Neo's default landing page at `/lobby`. It is where you choose a display name, browse public matches, and decide whether to play or watch. No account is required. The site root redirects here.
...
After choosing **Join** or **Create match**, you enter the match view. Before the game starts, it shows a waiting overlay — for example, **"Waiting for 2 more players… (1/3)"** — your accepted handle, and the seat count (shown as *k / N*). Keep the page open. The board appears and begins ticking when the match starts. The waiting message announces the transition for screen readers as well as visually.
...
- Order controls do not issue orders.
- You cannot change the match state.

Participant labels are handle-first and come from the server's accepted handles. If a handle is unavailable, the interface uses a generic label or a safe ID fallback; IDs are correlation data, not credentials. Labels are presented as separate, direction-safe fields, so a right-to-left or mixed-direction name cannot reorder surrounding text. Bearer credentials are never shown as labels, printed in diagnostics, or requested from players.
