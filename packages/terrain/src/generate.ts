...
function validateRequest(req: Readonly<TerrainGenerationRequest>): void {
    ...
    if (!Number.isInteger(boardSize) || boardSize < MIN_GENERATION_BOARD_SIZE) {
        throw new GenerationError(`generateBoard: boardSize must be an integer ≥ 8 (got ${String(boardSize)})`, {
    ...
    if (playerCount !== MIN_PLAYER_COUNT && playerCount !== THREE_PLAYER_COUNT && playerCount !== FOUR_PLAYER_COUNT) {
        throw new GenerationError(`generateBoard: playerCount must be 2, 3, or 4 (got ${String(playerCount)})`, {
            kind: 'invalid_request',
            attempts: 0,
            lastReport: null,
        });
    ...
    if (!Number.isInteger(seed) || seed < 0) {
        throw new GenerationError(`generateBoard: seed must be a non-negative integer (got ${String(seed)})`, {
            kind: 'invalid_request',
            attempts: 0,
            lastReport: null,
        });
    ...
    if (typeof req.rng !== 'function') {
        throw new GenerationError(`generateBoard: req.rng must be a callable PRNG (got ${String(typeof req.rng)})`, {
            kind: 'invalid_request',
            attempts: 0,
            lastReport: null,
        });
...
export function generateBoard(req: Readonly<TerrainGenerationRequest>): TerrainGenerationResult {
    ...
    for (let attempt = 0; attempt < settings.maxRegenAttempts; attempt++) {
        ...
        const symmetricCities = enforceCitySymmetry(placedCities, req.boardSize, req.boardSize, req.playerCount);
