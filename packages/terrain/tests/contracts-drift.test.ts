/**
 * Contract Drift Test — Feature 003, Polish-phase (T055)
 *
 ...
 * risk: an edit to one file but not the other silently
 * desynchronizes the contract surface.
 *
 ...
 *      is a breaking change to the terrain's documented contract;
 *      bump `TERRAIN_API_VERSION` and document in the spec's
 *      "Revision History").
...
function repoPath(relativePath: string): string {
    // packages/terrain/tests/contracts-drift.test.ts → 3 levels up = repo root
