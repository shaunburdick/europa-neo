...
describe('SC-001 determinism at 10k seeds (10x spec)', () => {
  it(
    `${String(SC_001_TRIALS)} trials: same seed + same PRNG state → byte-identical Board`,
    { timeout: 60_000 },
    () => {
  ...
  it(
    `${String(SC_001_TRIALS)} trials: distinct seeds produce distinct Board hashes (US2 acceptance scenario 2)`,
    { timeout: 60_000 },
    () => {
      const seeds = goldenSeeds(SC_001_TRIALS);
