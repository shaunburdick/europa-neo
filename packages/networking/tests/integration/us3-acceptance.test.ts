...
function hasCell(view: TickBroadcastPayload['view'], coord: Readonly<{ x: number; y: number }>): boolean {
...
describe('US3 acceptance (late-join spectating)', () => {
    it('Given a running match, When a spectator connects, Then they receive a full snapshot and all subsequent ticks unfiltered', {
        ...
        try {
            ...
            const coords = new Set(ack.view.visibleCells.map((cell) => `${cell.coord.x},${cell.coord.y}`));
