...
export function attachSpectator(
    ...
    if (!channel.spectatorsAllowed) {
        const error = new NetworkError('match_not_joinable', 'spectators are not enabled for this match');
 ...
 */
export function detachSpectator(channel: MatchChannel, connectionId: ConnectionId, deps: SpectatorDeps): boolean {
