...
export interface ConnectionOptions {
    ...
    readonly onEnvelope?: (connection: Connection, envelope: ProtocolEnvelope<NetworkPayload>) => void;
...
export class Connection {
    ...
    constructor(options: ConnectionOptions) {
        ...
        const perSecond = options.rateLimit?.ordersPerSecond ?? NETWORK_CONSTANTS.defaultOrdersPerSecond;
        const burstFactor = options.rateLimit?.burstFactor ?? NETWORK_CONSTANTS.defaultRateLimitBurstFactor;
        this.rateBucket = {
    ...
    sendError(code: ErrorCode, message: string, detail?: NetworkErrorDetail, nowMs?: number): void {
        ...
        this.send({ type: 'error', version: NETWORK_API_VERSION, seq: 0 as SequenceNumber, payload }, nowMs);
    }
