 ...
 */
async function readLiveOrThrow(
  page: Page,
): Promise<NonNullable<Awaited<ReturnType<typeof readLive>>>> {
      ...
      .poll(
        async () =>
          ...
          (await alice.evaluate(() => {
            const handle = (window as unknown as { __europaLive?: LiveHandleView }).__europaLive;
            return (
              handle?.store.getState().feedback.some((entry) => entry.kind === 'success') ?? false
            );
          })),
        { timeout: 10_000 },
      )
