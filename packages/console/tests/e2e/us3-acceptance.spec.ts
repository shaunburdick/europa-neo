 ...
 */
async function hoverSubcell(page: Page, cx: number, cy: number, fx: number, fy: number): Promise<void> {
    const box = await page.locator('#map').boundingBox();
    if (box === null) {
        throw new Error('#map has no bounding box');
    }
    await page.mouse.move(box.x + (cx + fx) * DEFAULT_CAMERA.zoom, box.y + (cy + fy) * DEFAULT_CAMERA.zoom);
}
