...
describe('ErrorBoundary (T085)', () => {
    test('renders children while healthy', async () => {
        const onError = vi.fn();
        await render(createElement(ErrorBoundary, { onError }, createElement('p', null, 'healthy subtree')));
    ...
    test('a throwing child surfaces the accessible fallback', async () => {
        ...
        const onError = vi.fn();
        await render(createElement(ErrorBoundary, { onError }, createElement(Bomb, { message: 'boom' })));
