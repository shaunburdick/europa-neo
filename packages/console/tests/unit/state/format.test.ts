...
describe('formatActionConfirmation (companion FR-007 surface)', () => {
    ...
    it.each([
        [{ kind: 'setPipe', cell: { x: 1, y: 2 }, direction: 'N' } as PlayerAction, `Pipe N at ${at(1, 2)}`],
        [{ kind: 'clearPipe', cell: { x: 1, y: 2 }, direction: 'S' } as PlayerAction, `Clear pipe S at ${at(1, 2)}`],
        [
            { kind: 'setPipesExclusive', cell: { x: 3, y: 4 }, direction: 'E' } as PlayerAction,
            `Exclusive pipe E at ${at(3, 4)}`,
        ],
        [{ kind: 'clearAllPipes', cell: { x: 3, y: 4 } } as PlayerAction, `Cleared all pipes at ${at(3, 4)}`],
        [{ kind: 'setReserves', cell: { x: 5, y: 6 }, percent: 7 } as PlayerAction, `Reserved 70% at ${at(5, 6)}`],
        [
            {
                kind: 'paratroop',
                source: { x: 1, y: 1 },
        ...
        [{ kind: 'gun', source: { x: 4, y: 5 }, target: { x: 6, y: 7 } } as PlayerAction, 'Gun fire (4, 5) → (6, 7)'],
