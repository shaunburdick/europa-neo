...
function cell(x: number, y: number, owner: 1 | 2 | null, troops: number, terrain: 'land' | 'water' = 'land'): CellView {
    return {
        coord: { x, y },
...
describe('localPreflightOrder (Q-U05)', () => {
    ...
    it('accepts reserves and pipes on owned visible cells; surrender always passes', () => {
        expect(
            localPreflightOrder({ kind: 'setReserves', player: 1, cell: { x: 5, y: 5 }, percent: 5 }, VIEW, 1),
        ).toBeNull();
        expect(
