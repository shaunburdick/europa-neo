...
class FakeWebSocket {
    ...
    deliver(type: string, payload: unknown): void {
        serverSeq += 1;
        const envelope = { type, version: '0.2.0', seq: serverSeq, payload };
...
async function establish(revision = 1): Promise<FakeWebSocket> {
    ...
    socket.open();
    socket.deliver('helloAck', { protocolVersion: '0.2.0', connectionId: 'conn-1', heartbeatIntervalMs: 60_000 });
...
describe('establish cycle', () => {
    it('sequences hello → lobbyIdentity(claim) → lobbySubscribe → ready', async () => {
        ...
        socket.deliver('helloAck', { protocolVersion: '0.2.0', connectionId: 'c1', heartbeatIntervalMs: 60_000 });
...
describe('server-issued identity adoption (feature 010 Clarifications v1.6)', () => {
    it('replaces the local bootstrap mint with the server-delivered id and persists it', async () => {
        ...
        nextSocket.open();
        nextSocket.deliver('helloAck', { protocolVersion: '0.2.0', connectionId: 'c2', heartbeatIntervalMs: 60_000 });
...
describe('snapshot revision gating', () => {
    ...
    it('re-adopts LOW revisions after a re-establish (server restart simulation)', async () => {
        ...
        socketB.open();
        socketB.deliver('helloAck', { protocolVersion: '0.2.0', connectionId: 'c2', heartbeatIntervalMs: 60_000 });
...
describe('privacy — no bearer credential leakage in URLs, logs, or errors', () => {
    it('never mutates the caller-supplied URL across initial connect and retries', async () => {
        ...
        socketA.open();
        socketA.deliver('helloAck', { protocolVersion: '0.2.0', connectionId: 'c1', heartbeatIntervalMs: 60_000 });
