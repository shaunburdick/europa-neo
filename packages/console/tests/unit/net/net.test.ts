...
describe('netEventFromEnvelope (T031)', () => {
    it('helloAck carries connectionId + heartbeat interval', () => {
        expect(
            netEventFromEnvelope(envelope('helloAck', { connectionId: 'c-1', heartbeatIntervalMs: 4000 }), CTX),
    ...
    it('snapshot resyncs; stale ticks are dropped; fresh ticks pass', () => {
        expect(netEventFromEnvelope(envelope('snapshot', { view: { tick: 77 }, tick: 77 }), CTX)).toEqual({
            kind: 'reconnected',
            view: { tick: 77 },
        });
        expect(netEventFromEnvelope(envelope('tick', { view: { tick: 49 }, tick: 49 }), CTX)).toBeNull();
    ...
    it('orderAck correlates seq→ActionId; unknown seqs are ignored', () => {
        expect(netEventFromEnvelope(envelope('orderAck', { seq: 7, result: { ok: true } }), CTX)).toEqual({
            kind: 'orderAck',
            actionId: 42,
            result: { ok: true },
        });
        expect(netEventFromEnvelope(envelope('orderAck', { seq: 999, result: { ok: true } }), CTX)).toBeNull();
    });

    it('terminal and error map through; pong is informational', () => {
        expect(netEventFromEnvelope(envelope('terminal', { result: { winner: 1 } }), CTX)).toMatchObject({
            kind: 'terminal',
        });
        expect(netEventFromEnvelope(envelope('error', { code: 'match_full', message: 'full' }), CTX)).toEqual({
            kind: 'error',
            code: 'match_full',
            message: 'full',
        });
