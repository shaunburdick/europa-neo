...
function cellWithPipes(coord: { x: number; y: number }, pipes: ReadonlySet<string>): Record<string, unknown> {
    return {
        coord,
...
describe('rehydrateEnvelopeViews — wire arrays become real Sets', () => {
    ...
    test('non-view envelopes pass through by reference untouched', () => {
        const pong = fromWireText(toWireText('pong', { clientTimeMs: 1, serverTimeMs: 2 } as NetworkPayload));
        expect(rehydrateEnvelopeViews(pong)).toBe(pong);

        const orderAck = fromWireText(toWireText('orderAck', { seq: 4, result: { ok: true } } as NetworkPayload));
