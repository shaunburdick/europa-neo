...
describe('WsMatchClient handshake', () => {
    it('sends hello as the first frame and resolves connect on helloAck', async () => {
        ...
        expect(hello.payload).toMatchObject({ protocolVersion: NETWORK_API_VERSION });
...
describe('WsMatchClient seat claim', () => {
    it('joinMatch requires greeted state', async () => {
        ...
        const client = createWsMatchClient({ webSocketFactory: () => socket });
        await expect(client.joinMatch({ matchId: 'm' as never, role: 'player', displayName: 'A' })).rejects.toThrow(
            /greeted/,
        );
    });
    ...
    it('rehydrates joinAck snapshot views', async () => {
        ...
        client.onMessage((envelope) => {
            if (envelope.type === 'joinAck') {
                joinView = (envelope.payload as unknown as { view: { visibleCells: Array<{ pipes: unknown }> } }).view;
            }
        });
        socket.deliver('joinAck', {
        ...
        await joining;
        const pipes = (joinView as { visibleCells: Array<{ pipes: unknown }> } | null)?.visibleCells[0]?.pipes;
        expect(pipes).toBeInstanceOf(Set);
    ...
    it('rehydrates the reconnect snapshot + replay window (US2 resync path)', async () => {
        ...
        client.onMessage((envelope) => {
            ...
            if (envelope.type === 'snapshot' || envelope.type === 'tick') {
                seenPipes.push(
                    (envelope.payload as unknown as { view: { visibleCells: Array<{ pipes: unknown }> } }).view
                        .visibleCells[0]?.pipes,
                );
            }
        });
