...
describe('paratroop / gun through the preflight chain (US3 interplay)', () => {
    ...
    test('stale cursor aim counts as centered → no launch (research.md §13 #3)', () => {
        const outcome = translateKey(baseArgs({ key: 'p', cursor: cursorIn(10, 10, 0.85, 0.15), cursorAgeMs: 5000 }));
...
describe('shouldIgnoreKeyEvent (typing-surface + chord guards)', () => {
    test('ignores default-prevented and auto-repeat events', () => {
        expect(shouldIgnoreKeyEvent({ defaultPrevented: true, repeat: false, target: null })).toBe(true);
        expect(shouldIgnoreKeyEvent({ defaultPrevented: false, repeat: true, target: null })).toBe(true);
    });

    test('ignores keys typed into interactive elements', () => {
        const button = document.createElement('button');
        expect(shouldIgnoreKeyEvent({ defaultPrevented: false, repeat: false, target: button })).toBe(true);
        const input = document.createElement('input');
        expect(shouldIgnoreKeyEvent({ defaultPrevented: false, repeat: false, target: input })).toBe(true);
    });

    test('ignores contentEditable hosts; plain containers pass', () => {
        const editable = document.createElement('div');
        editable.contentEditable = 'true';
        expect(shouldIgnoreKeyEvent({ defaultPrevented: false, repeat: false, target: editable })).toBe(true);
        const plain = document.createElement('div');
        expect(shouldIgnoreKeyEvent({ defaultPrevented: false, repeat: false, target: plain })).toBe(false);
        expect(shouldIgnoreKeyEvent({ defaultPrevented: false, repeat: false, target: null })).toBe(false);
    });
});
