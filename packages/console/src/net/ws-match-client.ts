...
export function createWsMatchClient(options: WsMatchClientOptions = {}): WsMatchClient {
    ...
    function startHeartbeat(): void {
        ...
        heartbeatTimer = setInterval(() => {
            if (socket === null || (connection !== 'greeted' && connection !== 'joined' && connection !== 'rejoined')) {
                return;
            }
            try {
    ...
    function log(level: 'debug' | 'info' | 'warn' | 'error', msg: string, ctx: Record<string, unknown>): void {
    ...
    function sendEnvelope(payload: NetworkPayload, type: ProtocolEnvelope<NetworkPayload>['type']): SequenceNumber {
        if (socket === null) {
    ...
    function dispatchInbound(envelope: ProtocolEnvelope<NetworkPayload>): void {
        ...
        switch (envelope.type) {
            case 'tick': {
                const payload = envelope.payload as Extract<NetworkPayload, { tick: number; view: unknown }>;
    ...
    function handleEnvelope(envelope: ProtocolEnvelope<NetworkPayload>): void {
        switch (envelope.type) {
            ...
            case 'orderAck': {
                ...
                const payload = envelope.payload as Extract<NetworkPayload, { seq: number; result: CommandResult }>;
