 ...
 * @param registry     PlayerRegistry for mapping numeric indices to string IDs.
...
export function resolveTerminal(
    ...
    // post-decay snapshot). Owner bytes are 0-based numeric indices;
    // map to string PlayerIds via the registry.
    ...
    for (let i = 0; i < state.troopCounts.length; i++) {
        ...
        const id = registry.idAtIndex(owner - 1);
