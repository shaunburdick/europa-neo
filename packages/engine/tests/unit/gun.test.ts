...
function buildWorld(size: number, board: Board, state: WorldState): World {
    return {
        ...
        board,
        players: [
            { id: 'test-p1' as PlayerId, displayName: 'P1', status: 'alive', citiesOwned: 0, troopsHeld: 0 },
            { id: 'test-p2' as PlayerId, displayName: 'P2', status: 'alive', citiesOwned: 0, troopsHeld: 0 },
        ],
        state,
        rngSeed: 1,
...
describe('resolveGun — FR-014 cost + damage + no movement', () => {
    it('costs gunCost from source and damages gunDamage from target', () => {
        ...
        const orders: Order[] = [{ kind: 'gun', player: 'test-p1' as PlayerId, source: { x: 3, y: 3 }, target: { x: 5, y: 3 } }];
...
describe('resolveGun — branch coverage: edge cases', () => {
    ...
    it('all input orders rejected: state reference preserved', () => {
        ...
        const state = emptyState(size);
        placeStack(state, size, 3, 3, 1, 100);
        const orders: Order[] = [{ kind: 'gun', player: 'test-p1' as PlayerId, source: { x: 99, y: 99 }, target: { x: 5, y: 3 } }];
        const result = resolveGun(state, board, CONSTANTS, orders, REGISTRY);
