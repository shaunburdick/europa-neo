...
describe('applyCommand — pipe commands', () => {
    ...
    it('setPipesExclusive replaces existing pipes on the cell', () => {
        ...
        const pipeOrders: Array<{
            ...
            { atTick: 0, order: { kind: 'setPipe', player: PLAYER_1, cell: { x: 1, y: 1 }, direction: 'E' } },
            { atTick: 0, order: { kind: 'setPipe', player: PLAYER_1, cell: { x: 1, y: 1 }, direction: 'N' } },
            {
                atTick: 1,
                order: { kind: 'setPipesExclusive', player: PLAYER_1, cell: { x: 1, y: 1 }, direction: 'S' },
            },
        ];
    ...
    it('clearAllPipes removes all pipes from a cell', () => {
        ...
        const orders = [
            ...
            {
                atTick: 1,
                order: { kind: 'clearAllPipes' as const, player: PLAYER_1, cell: { x: 1, y: 1 } },
            },
        ];
    ...
    it('deferred order kinds (paratroop, gun, setReserves, surrender) are accepted when prerequisites are met', () => {
        ...
        expect(
            applyCommand(w0, { kind: 'gun', player: PLAYER_1, source: { x: 1, y: 1 }, target: { x: 2, y: 1 } }).result
                .ok,
        ).toBe(true);
        expect(
            applyCommand(w0, { kind: 'setReserves', player: PLAYER_1, cell: { x: 1, y: 1 }, percent: 3 }).result.ok,
        ).toBe(true);
        expect(applyCommand(w0, { kind: 'surrender', player: PLAYER_2 }).result.ok).toBe(true);
    });
});
...
describe('tick — orchestrator', () => {
    ...
    it('orders from different players are sorted deterministically', () => {
        ...
        expect(recorded[0]?.order.player).toBe(PLAYER_1);
