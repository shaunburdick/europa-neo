...
describe('quickstart Q-001 — tick loop end-to-end', () => {
    ...
    it('two identical scripted runs produce byte-identical state', () => {
        ...
        expect(Array.from(a.finalWorld.state.troopCounts)).toEqual(Array.from(b.finalWorld.state.troopCounts));
        expect(Array.from(a.finalWorld.state.troopOwners)).toEqual(Array.from(b.finalWorld.state.troopOwners));
        expect(Array.from(a.finalWorld.state.pipeMasks)).toEqual(Array.from(b.finalWorld.state.pipeMasks));
        expect(Array.from(a.finalWorld.state.cityOwners)).toEqual(Array.from(b.finalWorld.state.cityOwners));
