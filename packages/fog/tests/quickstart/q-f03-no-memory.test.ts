...
import { P1 } from '../fixtures/ids';
import { buildWorldWithCities, buildWorldWithTroops, ownerByteFor, withVisibilityRadius } from '../fixtures/world';
...
describe('Q-F03 — no memory of previously seen terrain', () => {
    it('destroying the viewer stack empties the next view (no carried-over cells)', () => {
        const before = withVisibilityRadius(buildWorldWithTroops(16, [[8, 8, P1, 5]]), RADIUS);
        expect(computePlayerView(before, P1).visibleCells.length).toBeGreaterThan(0);
        ...
        const view = computePlayerView(after, P1);
    ...
    it('a stack marching out of range reverts the old region to unknown', () => {
        ...
        const viewBefore = computePlayerView(before, P1);
        ...
        const ownerByte = ownerByteFor(before, P1);
        const owners = new Uint8Array(before.state.troopOwners);
        const counts = new Uint32Array(before.state.troopCounts);
        owners[8 * 16 + 8] = 0;
        counts[8 * 16 + 8] = 0;
        owners[0 * 16 + 0] = ownerByte;
        counts[0 * 16 + 0] = 5;
        const after = {
            ...before,
        ...
        const viewAfter = computePlayerView(after, P1);
